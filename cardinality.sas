...
title "Percent Missing for %upcase(&dsn.)";
proc sgplot data = xtmp.cardinality_report_fnl;
	hbar variable_name / response = percent_miss datalabel categoryorder = respdesc;
	xaxis grid;
	yaxis grid;
run;
title;

title "Percent Unique for %upcase(&dsn.)";
proc sgplot data = xtmp.cardinality_report_fnl;
	hbar variable_name / response = percent_unique datalabel categoryorder = respdesc;
	xaxis grid;
run;
title;
